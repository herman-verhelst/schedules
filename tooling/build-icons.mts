...
`;
    ...
    // Generate iconDefinitions.ts
    const iconDefinitionsOutputPath = path.join(typeDir, "iconDefinitions.ts");

    const iconDefinitionsContent = `// This file is generated by npm run build:icons

export const iconDefinitions: Record<string, string> = {
${await Promise.all(
        files.map(async (file) => {
            ...
            return `  "${icon}": '${svg.toString().replace(/\n/g, " ").replace(/\s+/g, " ").trim()}',`;
        })
    ).then(entries => entries.join("\n"))}
};
`;

    const iconDefsChanged = await writeIfChanged(iconDefinitionsOutputPath, iconDefinitionsContent);
    if (iconDefsChanged) {
        logVerbose(`iconDefinitions.ts saved to ${path.relative(cwd, iconDefinitionsOutputPath)}`);
    }
